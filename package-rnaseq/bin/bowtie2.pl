...
bowtie2.pl - script to execute reference based alignment for input sequence file(s).

=head1 SYNOPSIS

    bowtie2.pl --r1 mate_1_sequence_file(s) [--r2 mate_2_sequence_file(s)] 
              --bi bowtie2_index_dir --p index_prefix [--o outdir] [--bb bowtie2_bin_dir] 
              [--num_mismatches mismatches_permitted] [--seedlen length] 
...
=head1 OPTIONS
    ...
    --bi <bowtie2_index_dir>        = /path/to/bowtie2_index directory.

    --p <index_prefix>             = prefix for index files.

    --o <output dir>               = /path/to/output directory. Optional. [present working directory]

    --bb <bowtie2_bin_dir>          = /path/to/bowtie2 binary. Optional. [/usr/local/bin]
...
The script executes the bowtie2 script from the bowtie2 software package.
...
GetOptions( \%hCmdLineOption,
            'seq1file|1=s', 'seq2file|2=s', 'bowtie2_index_dir|bi=s', 'prefix|p=s',
            'outdir|o=s', 'bowtie2_bin_dir|bb=s', 'samtools_bin_dir|sb=s', 'mode=s', 'num_mismatches=i',
...
$sCmd  = $hCmdLineOption{'bowtie2_bin_dir'}."/bowtie2";
#$sCmd .= " -".$hCmdLineOption{'mode'}." ".$hCmdLineOption{'num_mismatches'};
#$sCmd .= " -N"." ".$hCmdLineOption{'num_mismatches'};
#foreach $sKey ( keys %hCmdLineOption) {
    ...
    #next if (($sKey eq "seq1file") || ($sKey eq "seq2file") ||));
...
$sCmd .= " -x ".$hCmdLineOption{'bowtie2_index_dir'}."/".$hCmdLineOption{'prefix'};
...
$sCmd .= " ".$sReadFile." -S $sOutDir/$sPrefix.bowtie2.sam";
$sCmd .= " --quiet";
print($sCmd);
#chdir($sOutDir);
exec_command($sCmd);
...
($bDebug || $bVerbose) ? 
	print STDERR "\nConverting $sPrefix.bowtie2.sam to $sPrefix.bowtie2.bam ...\n" : ();

die "Error! $sOutDir/$sPrefix.bowtie2.sam doesn't exist.\n" if (! -e "$sOutDir/$sPrefix.bowtie2.sam");
sam2bam(\%hCmdLineOption, "$sOutDir/$sPrefix.bowtie2.sam", $sOutDir);

($bDebug || $bVerbose) ? 
	print STDERR "\nConverting $sPrefix.bowtie2.sam to $sPrefix.bowtie2.bam ... done\n" : ();
...
sub check_parameters {
    ...
    if ( (!(defined $phOptions->{'seq1file'})) ||
    	 (!(defined $phOptions->{'bowtie2_index_dir'})) ||
    	 (!(defined $phOptions->{'prefix'})) ) {
		pod2usage( -msg => $sHelpHeader, -exitval => 1);
	}
	
    ## handle some defaults
    $phOptions->{'bowtie2_bin_dir'} = BOWTIE2_BIN_DIR if (! (defined $phOptions->{'bowtie2_bin_dir'}) );
