...
bowtie2_build.pl - script to create Bowtie index for a reference FastA file

=head1 SYNOPSIS

    bowtie2_build.pl --r reference_fasta_file [--o outdir] [--b bowtie_bin_dir] 
...
=head1 OPTIONS
    ...
    --b <bowtie2_bin_dir>           = /path/to/bowtie_build binary. Optional. [/usr/local/bin]
...
GetOptions( \%hCmdLineOption,
            ...
            'bowtie2_bin_dir|b=s', 'args|a=s', 'verbose|v',
            'debug',
            'help',
            'man') or pod2usage(2);
...
#$sCmd  = $hCmdLineOption{'bowtie2_bin_dir'}."/bowtie2-build";
#$sCmd .= " ".$hCmdLineOption{'args'} if (defined $hCmdLineOption{'args'});
#$sCmd .= " -q" if (!($bDebug || $bVerbose));
#$sCmd .= " -f ".$sOutDir."/".$hCmdLineOption{'reffile'}.
		 " ".$sOutDir."/".$hCmdLineOption{'prefix'};

$sCmd  = $hCmdLineOption{'bowtie2_bin_dir'}."/bowtie2-build";
$sCmd .= " ".$hCmdLineOption{'args'} if (defined $hCmdLineOption{'args'});
#$sCmd .= " -q" if (!($bDebug || $bVerbose));
$sCmd .= " -f ".$hCmdLineOption{'reffile'}.
        " ".$sOutDir."/".$hCmdLineOption{'prefix'};
...
sub check_parameters {
    ...
    $phOptions->{'bowtie2_bin_dir'} = BIN_DIR if (! (defined $phOptions->{'bowtie2_bin_dir'}) );
}
