...
filter_deseq.pl -   filter DESeq output on [FDR, log fold change, Read Count Percentile, P value]
...
=head1 SYNOPSIS
 ...
 USAGE: filter_on_fc.pl
	...
	--filters = string containing combinations of various filters <FDR=0.5,RCP=0.1: FDR=0.3,RCP=.25:FDR=0.05,RCP=0.50>
                (RCP : Read count percentile [range 0-1])
...
my %worksheets;
my $map_info = {};
my ($file,$IN, $fh, $SA, $out, $summary_file, $summary_all, $i, $p, $stub, $sample_name, $sample1, $sample2, $total_output, $line, $workbook, $cmd, $t, $index);
my (@vals, @arr,  @parameters, @filters, @results_files, @files, @samples, @up, @down, @readcount, @sorted_readcount, @rc);
my %param;
my @su =();
my %outfiles = ();
my $f = 0;
...
foreach $file (@results_files) {
    ...
    while ($line = <$fh>) {
        next if ($line =~ /ID/);
        chomp($line);
        push (@readcount , (split(/\t/,$line))[1]) ;
    }
    @sorted_readcount = sort{$a<=>$b} @readcount ;
    close $fh;

	open($fh, "<", $file ) or die "Error Cannot open the deseq file : $file";
	...
	while (<$fh>) {
		...
		# if user mentioned map_file, we want to print name in output - 
	    if( $options{ 'map_file' } ) {
	    ...
	    for ($i = 0; $i< scalar @filters ;$i++) {
            ...
            if ($param{'DFC'} >= 0 ) {die "Error : DFC cutoff cannot be zero or positive" ;}
            if ($param{'UFC'} <= 0 ) {die "Error : UFC cutoff cannot be zero or negative" ;}
			if (defined $param{'RCP'} && ($param{'RCP'} < 0 || $param{'RCP'} > 1) ) {die "Error : Read Count Percentile needs to be in range 0-1" ;}

            if (! defined $param{'RCP'}) {
                $param{'RC'} = 0 ;
                $rc[$i] = 0;
            }
            else {
                $index = int(scalar (@sorted_readcount) * $param{'RCP'}) - 1;
                $param{'RC'} = $sorted_readcount[$index] ;
                $rc[$i] = $sorted_readcount[$index];
            }    
    ...
    for( $i = 0; $i < scalar @filters ;$i++) {
        ...
        if (! defined $param{'FDR'}) {$param{'FDR'} = 'NA' ;}
        if (! defined $param{'P'}) {$param{'P'} = 'NA' ;}
        if (! defined $param{'RCP'}) {$param{'RCP'} = 'NA';}
        if (! defined $param{'UFC'}) {$param{'UFC'} = 1 ;}
        if (! defined $param{'DFC'}) {$param{'DFC'} = -1 ;}
		$t = sprintf("%.2f", $rc[$i]);
        print $SA "$param{'FDR'}\t$param{'P'}\t$param{'RCP'} (RC=$t)\t$param{'UFC'}\t$param{'DFC'}\t$samples[$i]\t$up[$i]\t$down[$i]\t$p\n";
	}
...
sub check_parameters {
	...
	if (! defined $options{'filters'} ){
	    $options{'filters'} = "FDR=0.05,RCP=.25,UFC=1,DFC=-1";
	}
    }
