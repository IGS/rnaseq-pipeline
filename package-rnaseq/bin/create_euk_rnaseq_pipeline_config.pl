...
=head1 SYNOPSIS
    ...
    create_euk_rnaseq_pipeline_config.pl --s <samples_file> --c <config_file> --r <reference_fasta> 
                                         ...
                                         [--split][--alignment] [--idxfile <hisat2/bowtie_index>] [--visualization] 
...
=head1 OPTIONS
    ...
    --build_indexes                   = execute hisat2_build component, or bowtie_build if --tophat_legacy was specified. 
                                        Requires '--r'.
    ...
    --split                           = excute fastq split component for shorter alignement time
      ...
      --idxfile <hisat2/bowtie_index>   = /path/to/hisat2 or /path/to/bowtie index file for alignment of all samples.
                                        Required for '--alignment' if not specifying '--bowtie_build' or '--hisat2_build'.

   --alignment                       = execute HISAT2 alignment component (or tophat if --tophat_legacy).
    ...
    --cufflinks_legacy                = If enabled, will use CuffCompare and Cuffmerge for comparing and merging assemblies.
...
=head1  DESCRIPTION
    ...
    hisat2_build             : generates the hisat2 index files for a given reference file.
    fastx_toolkit            : generates the quality stats and the trimmed sequence file(s) for the given sequence file(s).
    hisat2                   : generates the HISAT@ alignment files for single-end or paired-end sequence(s).
...
GetOptions( \%hCmdLineOption,
			...
			'build_indexes', 'quality_stats', 'quality_trimming', 'split',  
			'alignment', 'idxfile=s', 'visualization', 'rpkm_analysis', 'annotation_format=s', 
...
my ($sFeature, $sAttrID);
my ($samtools_convert_position,$samtools_convert_name);
...
# Copy the sample file into the timestampped output dir (so we don't lose the original)
my (undef, undef, $sample_file_prefix) = File::Spec->splitpath($hCmdLineOption{'sample_file'});
my $cp_sample_file = $sOutDir . "/" . $sample_file_prefix;
if (defined $hCmdLineOption{'sample_file'}) {
	($bDebug || $bVerbose) ? print STDERR "\nCopying $hCmdLineOption{'sample_file'} to $cp_sample_file .....\n" : undef;
	copy($hCmdLineOption{'sample_file'}, $cp_sample_file) 
}
...
if ((defined $hCmdLineOption{'quality_stats'}) || 
	...
	open($fpSMPL, "<$cp_sample_file") or die "Error! Cannot open $cp_sample_file for reading: $!";
...
if (defined $hCmdLineOption{'build_indexes'}) {
    ...
    if (defined $hCmdLineOption{'tophat_legacy'}){
        ...
        # Add hisat2_build
	    if (defined ($hConfig{'hisat2_build'}{'HISAT2_INDEX_PREFIX'})) {
		    $sBwtIndexPrefix = $hConfig{'hisat2_build'}{'HISAT2_INDEX_PREFIX'}[0];
	    } else {
		    ($_, $_, $sBwtIndexPrefix) = File::Spec->splitpath($hCmdLineOption{'reffile'});
		    $sBwtIndexPrefix =~ s/.(\w+)$//;
		    $hConfig{'hisat2_build'}{'HISAT2_INDEX_PREFIX'}[0] = $sBwtIndexPrefix;
		    $hConfig{'hisat2_build'}{'HISAT2_INDEX_PREFIX'}[1] = "hisat2 index prefix";
	    }
	    ###	Add HISAT2 Build Component & Parameters ###
	    init_component($oPL, "serial");
            include_component_layout($oPL, $sTemplateDir, "hisat2_build", "reference");
	    complete_component($oPL);
	
	    %hParams = ();
	    $hParams{'INPUT_FILE'} = ["$hCmdLineOption{'reffile'}", "path to reference FastA file"];
	    config2params(\%hParams, \%hConfig, 'hisat2_build');
	    add_config_section($fpPC, "hisat2_build", "reference");
	    add_config_parameters($fpPC, \%hParams);
	
	    $sBwtIndexDir = '$;REPOSITORY_ROOT$;/output_repository/hisat2_build/$;PIPELINEID$;_reference/i1/g1';
	}
...
if (defined $hCmdLineOption{'alignment'} || defined $hCmdLineOption{'split'}) {
	...
	if (defined $hCmdLineOption{'split'}) {
		...
		if ($bPE) {
			...
			$hParams{'SAMPLE_INFO'} = ["$cp_sample_file", "path to sample info file with information on all samples to be analyzed"] if (defined $hCmdLineOption{'quality_trimming'});
	    ...
	    if (defined $hCmdLineOption{'tophat_legacy'}) {
		    ...
		    ###	HISAT2 Components ###
		    include_component_layout($oPL, $sTemplateDir, "hisat2", "alignment");
		
		    ###	Add HISAT2 Parameters ###
		    %hParams = ();
		    $hParams{'INPUT_FILE_LIST'} = ["$sListFile", "path to list file consisting of tab separated first mate and second mate sequence files"];
		    $hParams{'HISAT2_INDEX_DIR'} = ["$sBwtIndexDir", "path to HISAT2 package binary directory"];
		    $hParams{'HISAT2_INDEX_PREFIX'} = ["$sBwtIndexPrefix", "HISAT2 index prefix"];
	
		    $sArgs = "";
		    $sArgs = $hConfig{'hisat2'}{'OTHER_ARGS'}[0] if (defined $hConfig{'hisat2'}{'OTHER_ARGS'});
		    $sArgs .= " --solexa-quals" if (($hCmdLineOption{'quality'} == 64) && ($sArgs !~ m/--solexa-quals/));
		    $hConfig{'hisat2'}{'OTHER_ARGS'}[0] = $sArgs;
		    $hConfig{'hisat2'}{'OTHER_ARGS'}[1] = "Additional Arguments" if (! defined $hConfig{'hisat2'}{'OTHER_ARGS'});
		    config2params(\%hParams, \%hConfig, 'hisat2');
		    add_config_section($fpPC, "hisat2", "alignment");
		    add_config_parameters($fpPC, \%hParams);
		
		    $sListBamFile = '$;REPOSITORY_ROOT$;/output_repository/hisat2/$;PIPELINEID$;_alignment/hisat2.bam.list';
		}
	    ...
	    if (defined $hCmdLineOption{'tophat_legacy'}) {
		    ...
		    $sListFile = '$;REPOSITORY_ROOT$;/output_repository/hisat2/$;PIPELINEID$;_alignment/hisat2.bam.list';
	...
	if  (!(defined $hCmdLineOption{'split'})){
		...
		if (defined $hCmdLineOption{'tophat_legacy'}){
            ...
            ###accept the other args parameter Jan062022
            $sArgs = "";
		    $sArgs = $hConfig{'align_hisat2_stats'}{'OTHER_ARGS'}[0] if (defined $hConfig{'align_hisat2_stats'}{'OTHER_ARGS'});
            $hParams{'INPUT_FILE'} = ["$sMapStatsList", "path to list of mapstats files"];
            include_component_layout($oPL, $sTemplateDir, "align_hisat2_stats", "hisat2_stats");
		    $hConfig{'align_hisat2_stats'}{'OTHER_ARGS'}[0] = $sArgs;
            $hConfig{'align_hisat2_stats'}{'OTHER_ARGS'}[1] = "Additional Arguments" if (! defined $hConfig{'align_hisat2_stats'}{'OTHER_ARGS'});
            config2params(\%hParams, \%hConfig, 'align_hisat2_stats');
            add_config_section($fpPC, "align_hisat2_stats", "hisat2_stats");
		}
		add_config_parameters($fpPC, \%hParams);
	}
	else{
...
# Deciding how samtools output should be sorted
...
elsif ( $samtools_convert_position == 1){
	init_component($oPL, "serial");
	include_component_layout($oPL, $sTemplateDir, "samtools_file_convert", "sorted_position");
}
elsif ( $samtools_convert_name == 1){
	init_component($oPL, "serial");
	include_component_layout($oPL, $sTemplateDir, "samtools_file_convert", "sorted_name");
}
...
if ( $samtools_convert_position == 1){
	%hParams = ();
	if ((defined $hCmdLineOption{'file_type'}) && ($hCmdLineOption{'file_type'} =~ m/SAM/i)) {
...
if ((defined $hCmdLineOption{'isoform_analysis'}) || (defined $hCmdLineOption{'diff_isoform_analysis'})) {
	...
	if (defined $hCmdLineOption{'diff_isoform_analysis'}) {
		...
		if (! defined $hCmdLineOption{'use_ref_gtf'}) {
			...
			#create_cuffsuite_files.cuffcompare will work to create both Cuffcompare and Cuffmerge input
			...
			if (defined $hCmdLineOption{"cufflinks_legacy"}) {
            	...
            	include_component_layout($oPL, $sTemplateDir, "cuffmerge", "merge");
			}
			# Create the "create_cuffsuite_files" component config
            %hParams = ();
            $hParams{'SAMPLE_INFO'} = ["$cp_sample_file", "path to sample info file with information on all samples to be analyzed"];
			...
			# Create the "Cuffcompare" or "Cuffmerge" component config file
			...
			if (defined $hCmdLineOption{'cufflinks_legacy'}) {
				...
				$hParams{'REFERENCE_FASTA'} = ["$sSamRefFile", "path to reference FastA file"];
				config2params(\%hParams, \%hConfig, 'cuffmerge');
				add_config_section($fpPC, "cuffmerge", "merge");
				$sGtfFileList = '$;REPOSITORY_ROOT$;/output_repository/cuffmerge/$;PIPELINEID$;_merge/cuffmerge.combined.gtf.list';
			}
            add_config_parameters($fpPC, \%hParams);
		...
		if (defined $hCmdLineOption{'cufflinks_legacy'}) {
			...
			###	Add CuffQuant/CuffDiff/CuffNorm Component & Parameters below ###
			include_component_layout($oPL, $sTemplateDir, "create_cuffsuite_files", "cuffquant");
			include_component_layout($oPL, $sTemplateDir, "cuffquant", "profile");
			include_component_layout($oPL, $sTemplateDir, "create_cuffsuite_files", "cuffdiff");
            include_component_layout($oPL, $sTemplateDir, "cuffdiff", "differential_expression");
			include_component_layout($oPL, $sTemplateDir, "cuffnorm", "normalize");
		}
		include_component_layout($oPL, $sTemplateDir, "cuffdiff_filter", "filter_cuff");

		# If the latest version of cufflinks is used, we run cuffquant before cuffdiff
		if (! defined $hCmdLineOption{'cufflinks_legacy'}) {
			# Create cuffsuite files - cuffquant
			%hParams = ();
			$hParams{'SAMPLE_INFO'} = ["$cp_sample_file", "path to sample info file with information on all samples to be analyzed"];
			$hParams{'CUFF_PROG'} = ["Cuffquant", "Cuffsuite program (Cuffcompare, Cuffquant, or Cuffdiff) to create files for"];
			$hParams{'GRP_COMP'} = ["$hCmdLineOption{'comparison_groups'}", "string of groups to compare. e.g. \"GRP#2vsGRP#1,GRP#3vsGRP#1\""];
			if (defined $hCmdLineOption{'use_ref_gtf'}) {
				die "Error! Annotation file undefined !!!\n" if (!(defined $hCmdLineOption{'gtffile'}));
			...
			add_config_section($fpPC, "create_cuffsuite_files", "cuffquant");
			add_config_parameters($fpPC, \%hParams);
		
			# Cuffquant
			$sCuffFileList = '$;REPOSITORY_ROOT$;/output_repository/create_cuffsuite_files/$;PIPELINEID$;_cuffquant/cuffsuite_input_file.list';
			...
			config2params(\%hParams, \%hConfig, 'cuffquant');
			add_config_section($fpPC, "cuffquant", "profile");
			add_config_parameters($fpPC, \%hParams);
		}

		# Create_cuffsuite_files - cuffdiff
		%hParams = ();
		$hParams{'SAMPLE_INFO'} = ["$cp_sample_file", "path to sample info file with information on all samples to be analyzed"];
		$hParams{'CUFF_PROG'} = ["Cuffdiff", "Cuffsuite program (Cuffcompare, Cuffquant, or Cuffdiff) to create files for"];
		$hParams{'GRP_COMP'} = ["$hCmdLineOption{'comparison_groups'}", "string of groups to compare. e.g. \"GRP#2vsGRP#1,GRP#3vsGRP#1\""];
		if (defined $hCmdLineOption{'use_ref_gtf'}) {
			die "Error! Annotation file undefined !!!\n" if (!(defined $hCmdLineOption{'gtffile'}));
			$hParams{'GTFFILE'} = ["$hCmdLineOption{'gtffile'}", "path to single reference GTF file to be used with all cuffdiff comparisons"];
		...
		# Add cuffnorm if using modern cufflinks.  Uses same inputs as cuffdiff
		if (! defined $hCmdLineOption{'cufflinks_legacy'}) {
			%hParams = ();
			$hParams{'INPUT_FILE_LIST'} = ["$sCuffFileList", "path to list of tab-delimited lists of aligmment SAM file(s) and respective annotation GTF files"];
			config2params(\%hParams, \%hConfig, 'cuffnorm');
			add_config_section($fpPC, "cuffnorm", "normalize");
			add_config_parameters($fpPC, \%hParams);
		}
...
($bDebug || $bVerbose) ? print STDERR "\nProcessing $cp_sample_file ..... done\n" : undef;
